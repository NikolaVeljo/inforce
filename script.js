...
const generateQuery = (e) => {
    ...
    `IF NOT EXISTS (SELECT 1 FROM forms_definitions  WHERE   forms_definitions_uid = '${forms_definitions_uid.value.trim()}' and licensee_code = '${licensee_code.value.trim()}')
     ...
     INSERT  INTO forms_definitions ( forms_definitions_uid, licensee_code, add_date_time, added_by, begin_date, change_date_time, 
        ...
        VALUES ('${forms_definitions_uid.value.trim()}', '${licensee_code.value.trim()}', ${!add_date_time.value ? 'getdate()' : formatDate(add_date_time.value)}, '${added_by.value.trim()}', '${!begin_date.value ? 'getdate()' : formatDate(begin_date.value)}', ${!change_date_time.value ? 'getdate()' : formatDate(change_date_time.value)}, '${changed_by.value.trim()}', ${end_date.value === '' ? 'null' : "'" + formatDate(end_date.value) + "'"}, '${file_name.value.trim()}', '${form_code.value.trim()}', '${form_code_alt.value.trim()}', '${form_list_order.value === '' ? '0' : form_list_order.value}', '${form_name.value.trim()}', '${form_name_print.value.trim()}', '${form_number.value.trim()}', '${group_print_order.value.trim()}', '${include_in_forms_list.value}', '${include_on_policy_dec.value}', '${internal_use_only_flag.value}',
